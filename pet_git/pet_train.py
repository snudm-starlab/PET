...
from pet_trainer import PetTrainer
...
def main(cfg: FairseqConfig) -> None:
    ...
    if cfg.distributed_training.ddp_backend == "fully_sharded":
        ...
        if str(cfg.criterion._name).lower().startswith("ptp"):
            ...
            teacher_cfg.model.encoder_ffn_embed_dim = 1024
        ...
        ### build teacher model
        teacher_model=task.build_teacher_model(teacher_cfg.model)

    ### set criterion for each task; kd or pkd
    ...
    if cfg.common.model_parallel_size == 1:
        trainer = PetTrainer(cfg, task, model, criterion,teacher_model=teacher_model,quantizer= quantizer)
    else:
    ...
    trainer.load_teacher_checkpoint(f"./{teacher_dir}/checkpoint_best.pt")
    ...
    while epoch_itr.next_epoch_idx <= max_epoch:
        ...
        # train for one epoch
        valid_losses, should_stop = pet_train(cfg, trainer, task, epoch_itr)
        if should_stop:
            break
...
def pet_train(
    ...
    for i, samples in enumerate(progress):
        with metrics.aggregate("train_inner"), torch.autograd.profiler.record_function(
            ...
            log_output = trainer.train_step_kd(samples)
...
def get_valid_stats(
    ...
    if tracking_best and hasattr(checkpoint_utils.save_checkpoint, "best"):
        ...
        #epoch, alpha, beta, T
        crt=trainer.get_criterion()
        try:
            print("recording...")
            training_cond=[stats["num_updates"]]+crt.get_cond()
            write_results(stats[key], training_cond, "PKD")
        except:
            pass
