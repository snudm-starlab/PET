...
class PetMultiheadAttention(nn.Module):
    ...
    def _update_attn_mask(self, num_heads_to_keep):
        """ before forward (F.multi_head_attention) update attn_mask
            ...
            attn_mask: (bsz*num_heads, dim), -inf
        """
        print("Update attn_mask")
        #% compute importance score
        sorted_head_score = self._compute_importance_score()
        reserved_head=sorted_head_score[:num_heads_to_keep]
        attn_mask=torch.Tensor([])
        proj = self.k_proj
        #% update attn_mask with importance score
        for head_idx in range(self.num_heads):
            if head_idx in reserved_head:
                attn_mask = torch.cat([attn_mask,torch.zeros([self.head_dim,self.embed_dim],dtype=proj.type,
                                                             device=proj.device)],dim=0)
            else:
                attn_mask = torch.cat([attn_mask,torch.empty([self.head_dim,self.embed_dim],dtype=proj.type,
                                                             device=proj.device).fill(float("-inf"))],dim=0)
    ...
    def _compute_importance_score(self):
        ...
        # % update attn_head_importance_score for next time_step masking
        self.attn_head_importance_score[stacked_layer_idx] = importance_scores_of_layer_heads

        return sorted_importance_scores_of_layer_heads
    ...
    def _adaptive_prune_heads(self, reserve_head_index: List[Tuple[int, int]]):
        print(
            f"Origin params\n * #heads:{self.num_heads}/embed_dim:{self.embed_dim}/out_features{self.q_proj.out_features}")
        new_q_weight = []
        new_q_bias = []
        new_k_weight = []
        new_k_bias = []
    ...
    def forward(
        ...
        if (
            ...
            # we don't use pytorch MHA operation (0916 hyojin)
            ...
            if is_self_attn_shuffle == True: 
                return F.multi_head_attention_forward(
                    ...
                    k_proj_weight=self.q_proj.weight,
