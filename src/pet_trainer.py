...
class PetTrainer(object):
    ...
    trainer object for Knowledge distillation
    """

    def __init__(self, cfg: FairseqConfig, task, model, criterion, teacher_model=None, quantizer=None, teacher_cfg=None):
        ...
        self.cfg = cfg
        self.t_cfg = cfg if teacher_cfg is None else teacher_cfg
        self.task = task
    ...
    def should_save_checkpoint_on_current_rank(self) -> bool:
        """Indicates whether to save checkpoints_pkd on the current DDP rank."""
        if (
    ...
    def always_call_state_dict_during_save_checkpoint(self) -> bool:
        if self.is_fsdp and not self.cfg.distributed_training.use_sharded_state:
            # FSDP calls communication collective when consolidating checkpoints_pkd
            return True
        else:
            return False
    ...
    def load_checkpoint(
        ...
        if bexists:
            load_on_all_ranks = (
                ...
                # TPUs don't support broadcast yet, so load checkpoints_pkd
            ...
            try:
                ...
                if utils.has_parameters(self.get_criterion()):
                    print("criterion:",self.get_criterion())
        ...
        if bexists:
            ...
            try:
                if (
                    ...
                    self.teacher_model.set_num_updates(
                        state["optimizer_history"][-1]["num_updates"]
                    )
                ...
                # To get rid of registering different pruned version of Roberta, I use the argument --mha-heads-to-keep to prune the Roberta model into a pruned version which matches the pruned ckpt.
                if (
                    safe_hasattr(self.teacher_model, "args")
                    and safe_hasattr(self.teacher_model.args, "mha_heads_to_keep")
                    and self.teacher_model.args.mha_heads_to_keep != -1
                ):
                    logger.info(
                        f"Prune model: keep {self.teacher_model.args.mha_heads_to_keep} heads for each multihead attention module"
                    )
                    for layer in self.teacher_model.encoder.sentence_encoder.layers:
                        reserve_head_index = layer.self_attn._get_reserve_head_index(
                            num_heads_to_keep=self.teacher_model.args.mha_heads_to_keep
                        )
                        layer.self_attn._adaptive_prune_heads(
                            reserve_head_index=reserve_head_index
                        )
                ...
                if (
                    safe_hasattr(self.teacher_model, "args")
                    and safe_hasattr(self.teacher_model.args, "ffn_blocks_to_remove")
                    and self.teacher_model.args.ffn_blocks_to_remove != -1
                ):
                    logger.info(
                        f"Prune model: remove {self.teacher_model.args.ffn_blocks_to_remove} ffn blocks for each transformer layer"
                    )
                    for layer in self.teacher_model.encoder.sentence_encoder.layers:
                        remove_index = layer._get_fc_rank(
                            remove_num=self.teacher_model.args.ffn_blocks_to_remove
                        )
                        layer._prune_fc_layer(remove_index=remove_index)
                    logger.info(self.teacher_model)

                self.teacher_model.load_state_dict(
                    state["model"], strict=True, model_cfg=self.t_cfg.model
                )
                # save memory for later steps
                del state["model"]
                if utils.has_parameters(self.get_criterion()):
    ...
    @metrics.aggregate("train_kd")
    def train_step_kd(self, samples, raise_oom=False):
        ...
        for i, sample in enumerate(samples):  # delayed update loop
            ...
            try:
                with maybe_no_sync():
                    ...
                    loss, sample_size_i, logging_output = self.task.train_step_kd(
                        sample=sample,
                        model=self.model,
    ...
    def _set_seed(self):
        ...
        # reproducible results when resuming from checkpoints_pkd
